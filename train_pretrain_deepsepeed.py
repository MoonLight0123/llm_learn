...
if __name__ == "__main__":
    ...
    # 先初始化DeepSpeed引擎（此时会初始化分布式环境）
    engine, optimizer, _, _ = deepspeed.initialize(
        model=model,
        config=args.deepspeed_config,
        dist_init_required=True  # DeepSpeed负责初始化分布式
    )
    train_sampler = DistributedSampler(train_ds, shuffle=True, num_replicas=engine.world_size, rank=engine.local_rank)
    ...
    # 将DataLoader关联到引擎
    engine.set_training_data_loader(train_loader)
    # model, tokenizer = init_model(lm_config)
    # train_ds = PretrainDataset(args.data_path, tokenizer, max_length=lm_config.max_seq_len)
